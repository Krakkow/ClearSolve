...
- **Vs a single open** (hero faces exactly one opener, no callers) — key `cash|{tableSize}|{heroPosition}|100bb|vs-open:{openerPosition}`; actions fold / call / 3-bet. Curated per opener *tier* (early / mid / late=CO / btn / sb) and per hero role: **BB** (widest), **SB** (OOP, tighter), or **in-position**. In-position defense is now **defender-position aware** (`IP_DEF` keyed by defender tier btn/co/mid/early × opener tier), so a button defends wider than the CO, which defends wider than the HJ, etc. Blind defense is keyed by opener tier (not every individual opener seat) — a remaining v1 coarsening.
