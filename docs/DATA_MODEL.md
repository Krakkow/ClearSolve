...
- **RFI** (folds to hero, hero opens) by position — key `cash|{tableSize}|{heroPosition}|100bb|rfi`. Now served from the **solved-offline library** (`src/domain/generated/rfiLibrary.json`, produced by `npm run gen:library` / `scripts/genLibrary.ts`) when present — real *mixed* fold/raise/shove frequencies from a high-iteration CFR solve — falling back to the hand-curated range otherwise. This is the **offline generation pipeline** (ADR-009): the solver runs natively/off-thread with a shared equity matrix at high iteration counts (no browser limits), and the validated output is committed as a static asset.

**vs-open / vs-3-bet stay CURATED — solving them offline was evaluated and rejected.** Generating response charts via the current 2-player reduction produced *less* accurate output than the curated reference: vs-3-bet pot geometry is broken (opener "calls" ~61% of a 3-bet), in-position defense collapses across defender positions (the reduction can't see defender position), blind-vs-blind defends too tight (the realization edge assumes the opener is IP), and CO/BTN openers collapse (tier-based assumed ranges). RFI generation works only because of its deliberate position-aware realization-edge model. **Solver-generated response/multiway charts are gated on a better engine** (Rust→WASM with proper multiway / postflop), not more heuristics on the 2-player model. See README "Finding".
- **Vs a single open** (hero faces exactly one opener, no callers) — key `cash|{tableSize}|{heroPosition}|100bb|vs-open:{openerPosition}`; actions fold / call / 3-bet. Curated per opener *tier* (early / mid / late=CO / btn / sb) and per hero role: **BB** (widest), **SB** (OOP, tighter), or **in-position**. In-position defense is **defender-position aware** (`IP_DEF` keyed by defender tier btn/co/mid/early × opener tier), so a button defends wider than the CO, etc. Blind defense is keyed by opener tier — a remaining v1 coarsening.
- **Vs a 3-bet** (hero OPENED, first raise, then faces a single 3-bet) — key `cash|{tableSize}|{heroPosition}|100bb|vs-3bet:{threeBettorPosition}`; actions fold / call / 4-bet (`VS_3BET` keyed by hero-open tier × 3-bettor type blind/ip — hero continues wider vs a blind 3-bet). Reached via the store's `heroMode: 'open-vs-3bet'` (UI: "Hero opened, vs 3-bet" mode building a `raiseDepth: 2` betContext `[hero open, 3-bettor 3-bet]`). The projection's `buildComposite` now skips the hero's own raise but still counts it for bet-leveling, so the live fallback treats the re-raiser as a 3-bettor (not an opener).
