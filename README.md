...
- **Multiway is reduced to a 2-effective-player game.** A 2–9-handed table is collapsed to *hero vs one composite opponent* (the live opponents' ranges combined), with dead money and pot odds modeled correctly. To avoid the classic collapse error (one composite is too easy to beat, so defense comes out absurdly wide), multiway defense applies two corrections: the composite is **skewed toward stronger hands as the field grows** (an approximation of the *best of N* opponents you must beat — folds the weak offsuit), and weak **speculative hands get a see-flop realization haircut** — keyed on playability (the kicker is weighted double, so weak suited *and* dominated weak-kicker offsuit hands like A2o/K4o realize poorly OOP multiway and fold). This keeps heads-up-resolved spots genuinely trustworthy and multiway spots a clearly-labeled (and now sane) estimate.
...
**Done:** HU push/fold solver · HU preflop bet-tree CFR+ · generalized 2–9-handed preflop tool · full action taxonomy · scenario builder · default ranges + inline range editor · correct cold-call pot odds · position-calibrated multiway opens · honest trust labeling · **predefined chart cache (RFI solved-offline at ~100bb *and* ~200bb + vs-open defense + vs-3-bet, 6-max & 9-max) with live fallback** · **full Rust→WASM engine** (equity + CFR+, bit-identical, ~2.7×/3.7× faster) · **solve-quality selector (Fast/Balanced/Max)** · **depth-aware realization edge** · **realistic open-sizing** (open-jam gated to short stacks, ≤20bb — no phantom deep open-jams) · **multiway field model** (composite skewed toward the *best of N* — multiway defense no longer absurdly wide).

**Next (candidates):** more open/3-bet sizes per node (a true non-jam *large* raise option) · more stack-depth buckets · 4-bet pots · multi-threaded wasm · **full-hand analysis** (postflop continuation + hand-history import/replay — Epic E10 / PRD §23) · tournament/ICM · local persistence (save/load) · practice/drill mode · true multiway via the engine. Build the wasm with `npm run wasm:build` (needs the Rust toolchain); the prebuilt wasm is committed so the app builds without Rust.
